...
def rolling_linear_regression(X, y, sample_weight, t, threshold_date, ridge=0.0):
    ...
    #initial version with xrange
    #for year in xrange(first_year,last_year+1):
    #modified version with range
    ...
    # Form hindcast predictions for all remaining blocks (xrange changed in range)
    for year in range(first_year,last_year+1):
...
def forward_rolling_linear_regression(X, y, sample_weight, t, threshold_date, 
    ...
    # Form hindcast predictions for all blocks (range instead of xrange)
