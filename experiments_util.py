...
def subsetmask_new(df, mask_df = get_contest_mask()):
    """Subsets df to rows with lat,lon pairs included in both df and mask_df
    ...
    Returns:
        Subsetted dataframe
    """
    return pd.merge(df, mask_df, on=['latitude', 'longitude'], how='inner')
...
def get_measurement_variable(gt_id, shift=None):
    ...
    Args:
       gt_id: ground truth data string ending in one of the suffixes below, or
           containing the string "wind", "GPP" or "RECO", or one of the NCEP/NCAR variable names
    ...
    if "wind" or "GPP" or "RECO" in gt_id:
        return gt_id+suffix
...
def shift_df_new(df, shift=None, date_col='time', groupby_cols=['latitude', 'longitude']):
    """Returns dataframe with all columns save for the date_col and groupby_cols
    shifted forward by a specified number of days within each group
...
def load_measurement(file_name, mask_df=None, shift=None):
    ...
    Args:
       ...
       shift: (optional) Number of days by which ground truth measurements
          should be shifted forward; date index will be extended upon shifting
    """
    fn = fsspec.open_local(f'simplecache::gs://{file_name}')
...
def get_ground_truth(gt_id, multiflux_resolution = "monthly", mask_df=None, shift=None):
    ...
    Args:
       gt_id: string identifying which ground-truth data to return;
         ...
         "wide_contest_sst", "wide_wind_hgt_{}", "wide_wind_uwnd_{}", "wide_wind_vwnd_{}", "METAFLUX_GPP_RECO_{}", "CASM_soil_moisture"
    ...
    if gt_id.startswith("GPP") or gt_id.startswith("RECO") :
        print(gt_id)
        #ds = xr.open_dataset('gs://leap-persistent/jbusecke/data/library/test/dataflow/METAFLUX_GPP_RECO_' + multiflux_resolution + '.zarr')#, #engine='zarr', chunks={}) 
        ds = xr.open_dataset('gs://leap-persistent/data-library/metaflux-gpp-reco-monthly-595733423-4998601040-1/METAFLUX_GPP_RECO_' + multiflux_resolution + '.zarr', engine='zarr')
        ...
        if "GPP" in gt_id :
            ...
            gt = ds.RECO
        #Transform into Dataframe (pd.DataFrame(gt) doesn't work for 3D dimension)
        gt = xr.DataArray.to_dataframe(gt) #3D
        #reshape the columns
        if isinstance(gt.index, pd.MultiIndex):
            gt.reset_index(inplace=True)
            #We reduce the zone of experiment for calculation time reasons (function .apply in shift-df-#new very long otherwise)
        ...
        gt = gt[gt['latitude'].between(lat_restriction_left, lat_restriction_right)] 
        gt = gt[gt['longitude'].between(lon_restriction_left, lon_restriction_right)]
        
        #in order to remainn coherent, we keep all the longitudes betweennn 0 and 360 degrees (and not ##(-180/180)
        gt['longitude'] = np.where(gt['longitude']< 0, gt['longitude'] + 360, gt['longitude'])
        
        gt = shift_df_new(gt, shift, date_col='time', groupby_cols=['latitude', 'longitude'])
        gt = gt.sort_values(['latitude','longitude'])
        #print(gt)
        ...
        gt.rename(columns={gt.columns[0]: 'lat'}, inplace=True)
        gt.rename(columns={gt.columns[1]: 'lon'}, inplace=True)
        gt.rename(columns={gt.columns[2]: 'start_date'}, inplace=True)
        return(gt)
    
    elif gt_id.startswith('CASM'):
        print(gt_id)
        #ds = xr.open_dataset('gs://leap-persistent/data-library/casm-595733423-4997696883-1/CASM.zarr', engine='zarr', chunks={}) 
        #gt=ds.CASM_soil_moisture
        #gt=xr.DataArray.to_dataframe(gt)
        gt=pd.read_csv('data/new_features/casm_us_sm.csv')
        gt['date']=pd.to_datetime(gt['date'])
        print("Getting CASM_soil_moisture")
        if isinstance(gt.index, pd.MultiIndex):
            gt.reset_index(inplace=True)
            
        lat_restriction_left = 27 #restriction of data around the US (latitude South)
        lat_restriction_right = 49 #restriction of data around the US (latitude North)
        lon_restriction_left = -124 #restriction of data around the US (longitude West)
        lon_restriction_right = -94 #restriction of data aroud the US (longitude East)
        ...
        gt = shift_df_new(gt,shift, date_col='date', groupby_cols=['lat', 'lon'])
        gt = gt.sort_values(['lat','lon'])
        
        gt.rename(columns={'date': 'start_date'}, inplace=True)
        print('test de CASM',gt)
        return(gt)
    ...
    if True == True :
        ...
        gs_path = 'leap-persistent/alexandrerebiere/data/'
        gt_file = gs_path+"gt-"+gt_id+"-14d-{}-{}.h5".format(first_year, last_year)
...
def get_ground_truth_unaggregated(gt_id, mask_df=None, shifts=None):
    ...
    gt_file = gs_path+gt_id+"-1d-{}-{}.h5".format(first_year, last_year)
...
def get_climatology(gt_id, mask_df=None, shift=None):
    ...
    climatology_file = gs_path+"official_climatology-"+gt_id+"-1981-2010.h5"
...
def get_forecast(forecast_id, mask_df=None, shift=None):
    ...
    forecast_file = gs_path+forecast_id+"-{}-{}.h5".format(first_year, last_year)
...
def get_cfsv2(forecast_variable, target_horizon, mask_df=None):
    ...
    forecast_file = gs_path+"cfsv2-"+forecast_variable+"-"+target_horizon+".h5"
...
def get_measurement_lag(gt_id):
    ...
    if gt_id.endswith("mjo"):
        ...
        days_late = 1
    elif gt_id.startswith("GPP"):
        days_late = 1
    elif gt_id.startswith("RECO"):
        days_late = 1
...
def get_lat_lon_date_features(multiflux_resolution = "monthly", gt_ids=[], gt_masks=None, gt_shifts=None, forecast_ids=[], forecast_masks=None, forecast_shifts=None, anom_ids=[], anom_masks=None, anom_shifts=None, first_year = None):
    ...
    for gt_id, gt_mask, gt_shift in zip(gt_ids, gt_masks, gt_shifts):
        ...
        t = time.time()
        # Load ground truth data
        gt = get_ground_truth(gt_id, multiflux_resolution, gt_mask, shift=gt_shift)
        #print('ground', gt)
        # Discard years prior to first_year
    ...
    for anom_id, anom_mask, anom_shift in zip(anom_ids, anom_masks, anom_shifts):
       ...
       # Check if ground truth column already exists
        ...
        if df is None or gt_col not in df.columns:
            ...
            gt = get_ground_truth(anom_id, multiflux_resolution, anom_mask, shift=anom_shift)
            # Discard years prior to first_year
            gt = year_slice(gt, first_year = first_year)
            # Use outer merge to include union of (lat,lon,date_col)
            # combinations across all features
...
def get_lat_lon_date_multiflux(multiflux_resolution="monthly", id='GPP'):
    """Returns dataframe of features associated with (lat, lon, start_date)
    values
    ...
    lat_restriction_left = 20 #restriction of data around the US (latitude North)
    lat_restriction_right = 60 #restriction of data around the US (latitude North)
    lon_restriction_left = -130 #restriction of data around the US (longitude West)
    lon_restriction_right = -60 #restriction of data aroud the US (longitude West)
    ...
    #print(gt)
    gt = shift_df_new(gt, shift=44, date_col='time', groupby_cols=['latitude', 'longitude'])
    gt = gt.sort_values(['latitude','longitude'])
    #print(gt)
...
def get_date_features(multiflux_resolution, gt_ids=[], gt_masks=None, gt_shifts=None, first_year=None):
    ...
    for gt_id, gt_mask, gt_shift in zip(gt_ids, gt_masks, gt_shifts):
        ...
        t = time.time()
        # Load ground truth data
        gt = get_ground_truth(gt_id, multiflux_resolution, gt_mask, gt_shift)
        # Discard years prior to first_year
        gt = year_slice(gt, first_year = first_year)
        # If lat, lon columns exist, pivot to wide format
        #Never used in the case of contest tmp2m 56w
...
def get_date_multiflux(multiflux_resolution="monthly", first_year=None, mask_df=None, id='GPP'):
    #choose monthly or daily
    ds = xr.open_dataset('gs://leap-persistent/jbusecke/data/library/test/dataflow/METAFLUX_GPP_RECO_'+multiflux_resolution+'.zarr', engine='zarr', chunks={}) 
    df = None
    if id == 'GPP' :
        #load the data for Growth Primary Production
        gt = ds.GPP.time #remove .time for 3D
    elif id == 'RECO':
        gt = ds.RECO.time #it might be the same time because coming from the same dataset
    #Transform into Dataframe (pd.DataFrame(gt) doesn't work for 3D dimension)
    gt = pd.DataFrame(gt)
    #gt = xr.DataArray.to_dataframe(gt) 3D
    #reshape the columns
    ...
    #if mask_df is not None:
        # Restrict output to requested lat, lon pairs
        #df = subsetmask_new(df, mask_df)
    
    #We reduce the zone of experiment for calculation time reasons (function .apply in shift-df-new very long otherwise)
    #put the parameters in the function ? 
    
    #lat_restriction = [20 , 60] #restriction of data aroud the US (latitude North)
    #lon_restriction = [-130, -60]
    #gt = gt[gt['latitude'].isin(lat_restriction)] #only keep the values inside
    #gt = gt[gt['longitude'].isin(lon_restriction)] #only keep the valuesinside
    ...
    #print(gt.columns)
    t = time.time()
    #if 'latitude_shift44' in gt.columns and 'longitude_shift44' in gt.columns:
    if 'latitude_shift44' in gt.columns and 'longitude_shift44' in gt.columns:
        print('test2')
        #gt = pd.pivot_table(gt, values='GPP_shift44', index='time',columns=['latitude_shift44', 'longitude_shift44']).reset_index()
        gt = pd.pivot_table(gt, values='GPP', index='time',columns=['latitude', 'longitude']).reset_index()
        print(gt)
        gt = pd.DataFrame(gt.to_records())
        print(gt)
        gt.drop("index", axis=1, inplace=True)
        ...
        gt.rename(columns={gt.columns[0]: 'start_date'}, inplace=True)
        print(gt)
        gt.rename(columns=lambda x: x.replace('(','GPP_shift44' +'_('), inplace=True)
        print(gt)
    print("Elapsed: {}s".format(time.time() - t))
    gt.rename(columns={gt.columns[0]: 'start_date'}, inplace=True)
    #df.rename(columns={df.columns[0]: 'start_date'}, inplace=True)
    df = df_merge(df, gt, on="start_date")
    return(df)
...
def create_lat_lon_date_data(gt_id,
    ...
    gt_ids = other_lat_lon_date_features + other_lat_lon_date_features + other_lat_lon_date_features
    print("gt_ids", gt_ids)
    gt_shifts=other_start_deltas + [2*delta for delta in other_start_deltas] + [365]*len(other_lat_lon_date_features)
    print("gt_shifts", gt_shifts)
    anom_ids=[gt_id] + past_gt_ids + past_gt_ids + past_gt_ids
    print("anom_ids", anom_ids)
    anom_shifts=[None] + past_start_deltas + [2*delta for delta in past_start_deltas] + [365]*len(past_gt_ids)
    print("anom_shifts", anom_shifts)
    print("past_start_deltas",past_start_deltas)
    
    
    #lat_lon_date_data_GPP = get_lat_lon_date_multiflux(multiflux_resolution, id='GPP')
    
    #lat_lon_date_data_RECO = get_lat_lon_date_multiflux(multiflux_resolution, id='RECO')

    #print('GPP', lat_lon_date_data_GPP)
    #print('RECO', lat_lon_date_data_RECO)
    
    lat_lon_date_data = get_lat_lon_date_features(
        multiflux_resolution,
    ...
    print(lat_lon_date_data)
    

    #lat_lon_date_data = pd.merge(lat_lon_date_data, lat_lon_date_data_GPP, on=["lat", "lon", "start_date"], how="outer")
    #lat_lon_date_data = pd.merge(lat_lon_date_data, lat_lon_date_data_RECO, on=["lat", "lon", "start_date"], how="outer")
...
def create_date_data(gt_id,
    ...
    #date_data = get_date_features(gt_ids=date_features, gt_shifts=start_deltas,
                                  #first_year=get_first_year(gt_id))
    df_gt = get_date_features(multiflux_resolution, gt_ids=date_features, gt_shifts=start_deltas,
                                  first_year=get_first_year(gt_id))
    df_GPP = get_date_multiflux(multiflux_resolution, first_year=get_first_year(gt_id),  id='GPP')
    df_RECO = get_date_multiflux(multiflux_resolution, first_year=get_first_year(gt_id), id='RECO')
    date_data = pd.merge(df_gt, df_GPP, on="start_date", how="outer")
    date_data = pd.merge(date_data, df_RECO, on="start_date", how="outer")
    print(date_data)
